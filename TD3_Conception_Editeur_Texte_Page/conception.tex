...
% Yoann
{\huge TODO}
Comme dit précédemment, le ficher temporaire est chaîné grâce à des adresses virtuelles.
...
Lors d'une insertion, on ajoute la lignel'algorithme classique d'insertion d'un élément dans une liste chaînée est utilisé :
...
L'accès direct (lorsque l'utilisateur demande d'accéder à la ligne $l$, par exemple) est possible grâce à la table de \textsl{mapping}, qui donne pour chaque bloc la dernière ligne stockée dans celui-ci.
...
\\
	\hline
Couche adressage & Implémente le mécanisme d'adresse virtuelle pour l'accès direct
